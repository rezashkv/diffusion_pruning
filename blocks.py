...
class ResnetBlock2DGated(ResnetBlock2D):
    ...
    # @torch.no_grad()
    # def get_flops(self, inputs):
    #     gn_flops = count_ops_and_params(self.norm1, inputs)[0]
    #     prunable_flops, _ = count_ops_and_params(self, inputs) - gn_flops
    #     # prunable flops + skip connection flops which is a sum over all the elements in the input tensor
    #     total_flops = prunable_flops + 2 * inputs.numel() + gn_flops
    #
    #     hard_out = hard_concrete(self.gate.gate_f)
    #     current_prunable_flops = prunable_flops * hard_out.sum() / hard_out.numel()
    #     current_total_flops = current_prunable_flops + 2 * inputs.numel() + gn_flops
    #
    #     self.__total_flops = total_flops
    #     self.__current_total_flops = current_total_flops
    #     self.__prunable_flops = prunable_flops
    #     self.__current_prunable_flops = current_prunable_flops

        # return {"total_flops": total_flops, "current_total_flops": current_total_flops,
        #         "prunable_flops": prunable_flops, "current_prunable_flops": current_prunable_flops}
...
class BasicTransformerBlockGated(BasicTransformerBlock):
    ...
    # def get_flops(self, inputs):
    #     prunable_flops, _ = count_ops_and_params(self, inputs)
    #     return prunable_flops
