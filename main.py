...
from op_counter import (add_flops_counting_methods, start_flops_count, stop_flops_count, reset_flops_count,
...
def main():
    ...
    if args.pruning:
        ...
        with torch.no_grad():
            z0 = torch.randn(1, vae.config['latent_channels'], 64, 64)
            hidden_latents = torch.randn(1, text_encoder.config.max_position_embeddings, text_encoder.config.hidden_size)
            unet(sample=z0, timestep=100, encoder_hidden_states=hidden_latents)
            unet.__total_flops__ = unet.compute_average_flops_cost()[0]
